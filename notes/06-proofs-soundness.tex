...
\hypvitem{sat} From \hyp{consok}, $\pc=[]$ thus $\theta(\pc)=\vtrue$. Together with \hyp{rest} and \hyp{models}, this gives our goal \goal{consomp}.
\end{hypvlist}
...
\begin{align*}
	\code{get}&: \PMap(I, \mmdl) \rarr I \rarr I \times \mmdl.\St \\
	\code{set}&: \PMap(I, \mmdl) \rarr I \rarr \mmdl.\St \rarr \PMap(I, \mmdl)
\end{align*}

We pretty-print \code{get} and \code{set} as \ppgetc{\st,i}{i',\st_{i'}} and \ppset{\st,i,\st_i}{\st'}.
\begin{align*}
	\text{Given }
...
\end{align*}
\begin{mathparpagebreakable}
\inferrule[CPMapGetMatch]{(h,d)=\fn{unwrap}(\st) \\ i\in\dom(h) \\ \st_{i}=h(i)}{\ppgetc{\st, i}{i, \st_{i}}}
\and\inferrule[CPMapGetAdd]{(h,d)=\fn{unwrap}(\st) \\ i\notin\dom(h) \\ d\neq\bot \\ i\in d}{\ppgetc{\st, i}{i,\bot}}
\and\inferrule[CPMapGetBotDomain]{(h,d)=\fn{unwrap}(\st) \\ i\notin \dom(h) \\ d=\bot}{\ppgetc{\st,i}{i,\bot}}
...
\begin{mathparpagebreakable}
\inferrule[CPMapAction]{\ppgetc{\st,i}{i',\st_{i'}} \\ \ppexecc{\alpha}{\st_{i'},\ins}{o,\st'_{i'},\outs} \\ \ppset{\st,i',\st'_{i'}}{\st'}}{\ppexecc{\alpha}{\st,i::\ins}{o,\st',i'::\outs}}
...
We now re-define \code{get} and \code{set}, lifting them to the symbolic realm (note \code{set} is unchanged, as it does not perform any sort of matching).

\begin{align*}
	\code{get}&: \PMap(I, \mmdl) \rarr I \rarr \pset(I \times \mmdl.\Sst \times \Pc) \\
	\code{set}&: \PMap(I, \mmdl) \rarr I \rarr \mmdl.\Sst \rarr \PMap(I, \mmdl)
\end{align*}
We pretty-print \code{get} and \code{set} as \ppget{\sst,i}{i',\sst_{i'},\pc} and \ppset{\sst,i,\sst_i}{\sst'}.
\begin{align*}
	\text{Given }
	\fn{wrap}(h, d) &\defeq \begin{cases}
		\bot &\If~\dom(h)=\emptyset \land d=\bot\\
...
\end{align*}
\begin{mathparpagebreakable}
\inferrule[PMapGetMatch]{(h,d)=\fn{unwrap}(\sst) \\ i'\in\dom(h) \\ \sst_{i'}=h(i')}{\ppget{\sst, i}{i', \sst_{i'}, [i=i']}}
...
\end{align*}
\begin{mathparpagebreakable}
\inferrule[PMapAction]{\ppget{\sst,i}{i',\sst_{i'},\pc} \\ \ppexec{\alpha}{\sst_{i'},\sins}{o,\sst'_{i'},\souts,\pc'} \\ \ppset{\sst,i',\sst'_{i'}}{\sst'}}{\ppexec{\alpha}{\sst,i::\sins}{o,\sst',i'::\souts, \pc :: \pc'}}
...
\and\inferrule[PMapCons]{\ppget{\sst,i}{i',\sst_{i'},\pc} \\ \ppcons{\sst_{i'},\delta,\sins}{o,\sst'_{i'},\souts,\pc'} \\ \ppset{\sst,i',\sst'_{i'}}{\sst'} \\ \souts'=\fn{lift\_if\_miss}(o,i',\souts)}{\ppcons{\sst,\delta,i::\sins}{o,\sst',\souts', \pc::\pc'}}
...
\and\inferrule[PMapProd]{\ppget{\sst,i}{i',\sst_{i'},\pc} \\ \ppprod{\sst_{i'},\delta,\sins,\souts}{\sst'_{i'},\pc'} \\ \ppset{\sst,i',\sst'_{i'}}{\sst'}}{\ppprod{\sst,\delta,i::\sins,\souts}{\sst', \pc::\pc'}}
...
We introduce a pair of axioms for \code{get}, similar to OX and UX soundness of actions, to ensure that the symbolic function retrieves the right state if it exists in the concrete counterpart of the state. We also introduce an axiom relating to the soundness of \code{set}, ensuring that modifications preserve compatibility with regards to a concrete state.
...
\pfassume \begin{hypvlist}
\hypvitem{getsym} $\ppget{\sst,\sym i}{\sym i', \sst_{i'}, \pc} \land \expeval{\sym i}=i \land \expeval{\sym i'}=i' \land \theta(\pc)=\vtrue $
\end{hypvlist}
\pfprove \begin{goalvlist}
\goalvitem{getcon} $\forall \st\ldotp \theta,s,\st \models \sst \implies \exists \st_{i'} \ldotp \ppgetc{\st, i}{i', \st_{i'}} \land \theta,s,\st_{i'} \models \sst_{i'}$
\end{goalvlist}
...
\begin{hypvlist}
\hypvitem{rule1} Assume $(\sym h,\sym d)=\fn{unwrap}(\sst) \land \sym i'\in\dom(\sym h) \land \sst_{i'}=\sym h(\sym i')$
\hypvitem{apply1} From \hyp{rule1}, \hyp{getsym} and \textsc{PMapGetMatch}, we have $\pc=[\sym i=\sym i']$
\hypvitem{pctrue1} From \hyp{getsym} and \hyp{apply1}, we get $\expeval{\sym i} = \expeval{\sym i'} = i = i'$
...
\hypvitem{evalheap} From \hyp{models}, \hyp{pctrue1} and the definition of $\models$, we have $i\in h$ such that $\st_i=h(i)$ and ${\theta,s,\st_i\models\sst_{i'}}$
\hypvitem{applycget} From \hyp{evalheap} we can apply \textsc{CPMapGetMatch}, thus \ppgetc{\st,i}{i,\st_i}. Together with \hyp{evalheap} this gives our goal \goal{getcon}.
\end{hypvlist}
...
\hypvitem{apply2} From \hyp{rule2}, \hyp{getsym} and \textsc{PMapGetAdd}, we have $\sym i'=\sym i$, $\sst_{i'}=\bot$ and ${\pc=[\sym i\notin \dom(\sym h)\land \sym i \in \sym d]}$%
...
\hypvitem{applycget2} From \hyp{pctrue2} and \hyp{notbot2} we can apply \textsc{CPMapGetAdd}, thus \ppgetc{\st,i}{i,\bot}.
\hypvitem{botmodels} From the definition of $\models$, $\theta,s,\bot\models\bot$, which along with \hyp{applycget2} completes our goal \goal{getcon}.
\end{hypvlist}
...
\hypvitem{apply3} From \hyp{rule3}, \hyp{getsym} and \textsc{PMapGetBotDomain}, we have $\sym i'=\sym i$, $\sst_{i'}=\bot$ and ${\pc=[\sym i\notin \dom(\sym h)]}$
...
\hypvitem{applycget3} From \hyp{pctrue3} and \hyp{botdom} we can apply \textsc{CPMapGetBotDomain}, thus ${\ppgetc{\st,i}{i,\bot}}$.
\hypvitem{botmodels3} From the definition of $\models$, $\theta,s,\bot\models\bot$, which along with \hyp{applycget3} completes our goal \goal{getcon}.
\end{hypvlist}

\pfprop{Get Completeness}

\pfassume \begin{hypvlist}
\hypvitem{getcon} $\theta,s,\st \models \sst \land \ppgetc{\st, i}{i', \st_{i'}}$
\end{hypvlist}
\pfprove \begin{goalvlist}
\goalvitem{getsym} $\exists \sst_{i'}, \sym i, \sym i', \pc \ldotp \ppget{\sst,\sym i}{\sym i', \sst_{i'}, \pc} \land \theta,s,\st_{i'}\models\sst_{i'} \land \expeval{\sym i}=i \land \expeval{\sym i'}=i' \land \theta(\pc)=\vtrue$
\end{goalvlist}
...
\hypvitem{apply1} From \hyp{rule1}, \hyp{getcon} and \textsc{CPMapGetMatch} we have $i'=i$.
...
\hypvitem{apply2} From \hyp{rule2}, \hyp{getcon} and \textsc{CPMapGetAdd}, we have $i'=i$ and $\st_{i'}=\bot$.
...
\hypvitem{apply3} From \hyp{rule3}, \hyp{getcon} and \textsc{CPMapGetBotDomain}, we have $i'=i$ and $\st_{i'}=\bot$.
...
\begin{hypvlist}
\hypvitem{isok} If the action gets executed succesfully (it is not an out of bounds error), from \textsc{CPMapAction} we have $\ppgetc{\st,i}{i',\st_{i'}}$,  $\ppexecc{\alpha}{\st_{i'},\ins'}{o,\st'_{i'},\outs'}$ and $\ppset{\st,i',\st'_{i'}}{\st'}$, with $\ins=i::\ins'$ and $\outs=i'::\outs'$
\hypvitem{getcomp} From \ref{eq:get-complete}, \hyp{conex} and \hyp{isok}, we have $\exists \sst_{i'}, \sym i, \sym i', \pc \ldotp \ppget{\sst,\sym i}{\sym i', \sst_{i'}, \pc} \land \theta,s,\st_{i'}\models\sst_{i'} \land \expeval{\sym i}=i \land \expeval{\sym i'}=i' \land \theta(\pc)=\vtrue$
\hypvitem{subcact} From \hyp{getcomp} and \hyp{sissound}, we have $\exists \sins',\souts',\sst'_{i'},\pc',\theta'\ldotp \ppexec{\alpha}{\sst_{i'},\sins'}{o,\sst'_{i'},\souts',\pc'}$. 
...
\begin{hypvlist}
\hypvitem{isok} If the action gets executed succesfully (it is not an out of bounds error), from \textsc{PMapAction} we have $\ppget{\sst,i}{i',\sst_{i'},\pc'}$, $\ppexec{\alpha}{\sst_{i'},\sins'}{o,\sst'_{i'},\souts',\pc''}$ and ${\ppset{\sst,i',\sst'_{i'}}{\sst'}}$, with $\sins=i::\sins'$, $\souts=i'::\souts'$ and $\pc=\pc'::\pc''$
\hypvitem{execcon} From \hyp{sissound}, \hyp{isok} and \hyp{symex}, we get $\exists \st_{i'}\ldotp \theta,s,\st_{i'}\models\sst_{i'} \land \ppexecc{\alpha}{\st_{i'},\ins'}{p,\st',\outs'}$
\hypvitem{modelsbefore} From the definition of $\models$, it follows that if $\theta,s,\st'\models\sst'$ where $\ppset{\sst,i',\sst'_{i'}}{\sst'}$, then if $\theta,s,\st_{i'}\models\sst_{i'}$ there exists an $\st$ such that $\ppset{\st,i',\st_{i'}}{\st'}$, giving us $\theta,s,\st\models\sst$
\hypvitem{getsound} From \hyp{isok}, \hyp{symex} and \hyp{modelsbefore} we can use \ref{eq:get-sound}, giving us ${\exists \st_{i'} \ldotp \ppgetc{\st, i}{i', \st_{i'}}} \land \theta,s,\st_{i'} \models \sst_{i'}$
...
\hypvitem{notoutofbounds} If the action is not out of bounds, we have $\ppexecc{\alpha}{\st_{i'},\ins'}{o,\st_{i'}',\outs'}$ with $\ins=i::\ins'$ and $\outs=i::\outs'$.
\hypvitem{splitout} If $\st_f$ only includes the domain set and indices different than $i$, then the action is executed on the same $\st_{i'}$ and gives the same outcomes -- it can then be composed with the result, giving our goal \goal{framesub}.
...
\begin{hypvlist}
\hypvitem{subcon} From \hyp{consok} and \textsc{PMapCons}, given $\sins=\sym i::\sins'$ and $\pc=\pc'::\pc''$ we have $\ppget{\sst,\sym i}{\sym i', \sst_{i'}, \pc'}$, ${\ppcons{\sst_{i'},\delta,\sins'}{o,\sst_{i'}',\souts,\pc''}}$ and $\ppset{\sst,\sym i',\sst_{i'}'}{\sst_f}$.
...
\hypvitem{submodels} From \hyp{assume} and \ref{eq:get-sound}, we know $\exists \st_{i'}\ldotp \ppgetc{\st,i}{i',\st_{i'}} \land \theta,s,\st_{i'}\models\sst_{i'}$.
\hypvitem{subfmodels} From \hyp{assume}, \hyp{subcon} and the definition of \code{set} it follows that $\exists \st_{i'}'\ldotp \theta,s,\st_{i'}'\models\sst_{i'}'$.
\hypvitem{pcok} From \hyp{assume} and \hyp{subcon}, we have $\theta(\pc'')=\vtrue$.
\hypvitem{subsound} From \hyp{submodels}, \hyp{subfmodels}, \hyp{pcok} and \hyp{ssatsconssound} we can apply \ref{eq:consume-sound}, giving us $\exists \st_{\delta,i'}\ldotp \st_{\delta,i'}\disj \st_{i'}' \land \st_{i'}=\st_{\delta,i'}\cdot\st_{i'}' \land \theta,s,\st_{\delta,i'}\modelsp \corepred{\delta}{\ins'}{\outs}$.
...
\hypvitem{submodels} From \hyp{assume}, \hyp{subcon} and the definitions of $\models$  and composition we have $\exists \st_{i'}'\ldotp {\theta,s,\st_{i'}'\models\sst_{i'}'} \land \st_{i'}' \disj \st_{\delta,i'}$.
\hypvitem{subcomp} From \hyp{submodels}, \hyp{modelsp} and \hyp{ssatsconscomp}, we can apply \ref{eq:consume-complete}, giving us $\exists \st_{i'}\ldotp {\st_{i'}=\st_{\delta,i'}\cdot\st_{i'}'} \land \theta,s,\st_{i'}\models \sst_{i'} \land \theta(\pc'')=\vtrue$.
\hypvitem{modelscell} From the definition of $\models$, because \consume{} doesn't affect any other cell, given $\theta,s,\st'_{i'}\models\sst'_{i'}$ and $\theta,s,\st_f\models\sst_f$, we obtain $\theta,s,\st\models\sst$.
...
\hypvitem{exgetc} From \hyp{sthasi} and \hyp{modelscell}, it follows that we can apply \textsc{CPMapGetMatch} such that ${\ppgetc{\st,i}{i',\st_{i'}}}$. We can then use \ref{eq:get-complete}, giving us, from \hyp{subcon} that $\theta(\pc')=\vtrue$.
...
Given \consume{} for $\delta\in\preds_\mmdl$ only returns \Ok{} if the underlying's \consume{} returns \Ok. It follows that if that is the case, then by \hyp{ssatsconsox} there exists an execution of of \consume{} that succeeds for $\mmdl$, that is lifted by \PMap, giving us our goal \goal{exok}.
...
\begin{hypvlist}
\hypvitem{useprod} Given \hyp{prod}, from \textsc{PMapProd} we have $\ppget{\sst_f,\sym i}{\sym i',\sst_{f,i'},\pc'} \land \ppprod{\sst_{f,i'},\delta,\sins',\souts}{\sst_{i'},\pc''} \land \ppset{\sst_f,\sym i',\sst_{i'}}{\sst}$, given $\sins=\sym i::\sins'$ and $\pc=\pc'::\pc''$.
...
\hypvitem{submodels} From the definition of \code{set}, \hyp{useprod} and \hyp{assume}, we have $\exists \st_{i'}\ldotp \theta,s,\st_{i'}\models \sst_{i'}$.
\hypvitem{subprod} From \hyp{useprod}, \hyp{submodels}, \hyp{pcsok} and \hyp{ssatsprodsound}, we can apply \ref{eq:produce-sound}, giving us $\exists \st_{\delta,i'}, \st_{f,i'}\ldotp \st_{\delta,i'}\disj\st_{f,i'} \land \st_{i'}=\st_{\delta,i'}\cdot\st_{f,i'} \land \theta,s,\st_{\delta,i'}\modelsp \corepred{\delta}{\sins'}{\souts} \land \theta,s,\st_{f,i'}\models\sst_{f,i'}$.
...
\hypvitem{getexists} Given the fact $\st_f\disj\st_\delta$ and $\st_\delta\modelsp\corepred{\delta}{\sym i::\sins'}{\souts}$, it means the index $i$ is compatible with $\st_f$, so the state located at $i$ is obtainable via \code{get}. We thus have $\exists \st_{f,i} \ldotp \ppgetc{\st_f,i}{i',\st_{f,i}}$.
\hypvitem{usegetcomp} From \hyp{getexists} and \hyp{models}, we can use \ref{eq:get-complete}, giving us $\exists \sst_{f,i},\sym i,\sym i', \pc'\ldotp \ppget{\sst_f,\sym i}{\sym i',\sst_{f,i},\pc} \land \theta,s,\st_{f,i}\models\sst_{f,i} \land \expeval{\sym i}=i \land \expeval{\sym i'}=i' \land \theta(\pc')$.
...
\hypvitem{models2} From \hyp{stddef2}, \hup{stfdef2} and composition rules we have we have $\st_f\cdot\st_\delta=(h,d)$. It follows from \hyp{applyprod} that we have $\sst=(\sym h,\sym d)$ such that $\theta,s,(\st_f\cdot\st_\delta)\models\sst$. This along with \hyp{applyprod} and \hyp{pcok2} gives our goal \goal{prodcomp}.
\end{hypvlist}
