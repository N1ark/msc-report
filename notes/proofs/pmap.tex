...
\quad \ppget{\sst,\sym i}{\sym i', \sst_i, \pc} \land \theta,s,\st_i\models\sst_i \land \expeval{\sym i}=\expeval{\sym i'}=i \land \SAT(\pc)
\end{array}
\end{equation}
...
\hypvitem{pctrue2} From \hyp{getsym} $\SAT(\pc)$, thus from \hyp{apply2} and \hyp{models2}, $i\notin \dom(h) \land i\in d$
...
\hypvitem{pctrue3} From \hyp{getsym} $\SAT(\pc)$, thus from \hyp{apply3} and \hyp{models3}, $i\notin \dom(h)$.
...
\hypvitem{complete1} From \hyp{applys1} it follows that $\SAT([\sym i=\sym i])$, which along with \hyp{exsym1} gives our goal \goal{getsym}.
\end{hypvlist}
...
\hypvitem{getcomp} From \ref{eq:get-complete}, \hyp{conex} and \hyp{isok}, we have $\exists \sst_i, \sym i, \sym i', \pc \ldotp \ppget{\sst,\sym i}{\sym i', \sst_i, \pc} \land \theta,s,\st_i\models\sst_i \land \expeval{\sym i}=\expeval{\sym i'}=i \land \SAT(\pc)$
\hypvitem{subcact} From \hyp{getcomp} and \hyp{sissound}, we have $\exists \sins',\souts',\sst'_i,\pc',\theta'\ldotp \ppexec{\alpha}{\sst_i,\sins'}{o,\sst'_i,\souts',\pc'}\land \SAT[\theta',s](\pc') \land \theta',s,\st_i'\models\sst_i' \land \expeval{\sins'}=(\ins',\pc_i') \land \expeval[s,\pc_i']{\souts'}=(\outs',\pc_i'')$.%
...
\hypvitem{assume2} Assume $\theta,s,\st\models \sst \land \theta,s,\st_f\models \sst_f \land \SAT(\pc)$.
...
\hypvitem{exgetc} From \hyp{sthasi} and \hyp{modelscell}, it follows that we can apply \textsc{CPMapGetMatch} such that ${\ppgetc{\st,i}{\st_i}}$. We can then use \ref{eq:get-complete}, giving us, from \hyp{subcon} that $\SAT(\pc')$.
...
\hypvitem{assumed} Assume $\SAT(\pc) \land \theta,s,\st\models\sst$.
