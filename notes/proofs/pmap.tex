...
\begin{mathpar}
\inferrule[PMapSymInterpretation]{\forall \sym i\in\dom(\sym h)\ldotp \expeval{\sym i}=i \land i\in\dom(h) \land \theta,s,h(i) \models \sym h(\sym i) \\ \expeval{\dom(\sym h)}=\dom(h) \\ \expeval{\sym d}=d}{\theta,s,(h,d)\models (\sym h, \sym d)}
\end{mathpar}
...
\hypvitem{assume} Assume $\theta,s,\st\models \sst \land \theta(\pc)=\vtrue$.
