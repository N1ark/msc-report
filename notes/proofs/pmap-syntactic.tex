...
\inferrule[SyntacticPMapGetMatch]{(h,d)=\mathit{unwrap}(\sst) \\ i\in\dom(h) \\ \sst_i=h(i)}{\ppget{\sst, i}{i, \sst_{i}, []}}
\and\inferrule[SyntacticPMapGetBranch]{(h,d)=\mathit{unwrap}(\sst) \\ i\notin\dom(h) \\ i'\in\dom(h) \\ \sst_{i'}=h(i')}{\ppget{\sst, i}{i', \sst_{i'}, [i=i']}}
\and\inferrule[SyntacticPMapGetAdd]{(h,d)=\mathit{unwrap}(\sst) \\ i\notin\dom(h) \\ d\neq\bot}{\ppget{\sst, i}{i,\bot, [i\notin\dom(h) \land i\in d]}}
\and\inferrule[SyntacticPMapGetBotDomain]{(h,d)=\mathit{unwrap}(\sst) \\ i\notin \dom(h) \\ d=\bot}{\ppget{\sst,i}{i,\bot,[i\notin\dom(h)]}}
...
\hypvitem{complete1} From \hyp{applys1} it follows that $\theta([])=\vtrue$, which along with \hyp{exsym1} gives our goal \goal{getsym}.
\end{hypvlist}
