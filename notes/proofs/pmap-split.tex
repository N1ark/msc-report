...
\hypvitem{evalheap1} From \hyp{models1}, \hyp{rule1} and the definition of $\models$, we have $i\in \dom(h)$ such that $\st_i=h(i)$ and ${\theta,s,\st_i\models\sst_i}$.
...
\hypvitem{evalheap3} From \hyp{rule3}, \hyp{models3} and the definition of $\models$, we have $i\in \dom(h)$  such that $\st_i=h(i)$ and ${\theta,s,\st_i\models\sst_i}$.
...
\hypvitem{models4} Let $\st$ such that $\theta,s,\st\models\sst$.
\hypvitem{notbot4} From \hyp{rule4} we know $\sst\neq\bot$, thus $\st$ is of the form $\st=(h,d)$.
\hypvitem{notinh4} From \hyp{getsym} and \hyp{apply4}, we know $\theta(\pc)=\vtrue$, such that from \hyp{models4} we have ${i\notin \dom(h)} \land {i\in d}$.
...
\hypvitem{models5} Let $\st$ such that $\theta,s,\st\models\sst$.
\hypvitem{botdom} From \hyp{rule5} we have $\sym d=\bot$, so given $(h,d)=\fn{unwrap}(\st)$ and \hyp{models5}, we have $d=\bot$.%
\hypvitem{pctrue5} From \hyp{getsym} $\theta(\pc)=\vtrue$, thus from \hyp{apply5} and \hyp{models5}, $i\notin \dom(h)$.
...
\hypvitem{isinsomeheap} It follows from \hyp{getcon} and the definition of $\models$ that $\sst=(\sym h_c, \sym h_s, \sym d)$ such that there exists an $\sym i$ with $\expeval{\sym i}=i$, and a binding from $\sym i$ is in either the concrete or the symbolic part of the heap. We also define $\sym i$ as the value of $i$ directly lifted to symbolic values, such that $\code{is\_concrete}~i$.
\hypvitem{isincpart} From \hyp{isinsomeheap}, assume the binding is in the concrete part of the heap, $\sym h_c$. From this and \hyp{isinsomeheap}, we can apply \textsc{SplitPMapGetMatchCon}, giving us $\ppget{\sst,\sym i}{\sym i, \sym h_c(\sym i), []}$. This, along with the fact $\theta([])=\vtrue$, gives our goal \goal{getsym}.
\hypvitem{isincpart} Assume now from \hyp{isinsomeheap} that the binding is in the symbolic part of the heap, $\sym h_s$. From this and \hyp{isinsomeheap}, we can apply \textsc{SplitPMapGetMatchSym}, giving us $\ppget{\sst,\sym i}{\sym i, \sym h_s(\sym i), []}$. This again gives our goal \goal{getsym}.
\end{hypvlist}
...
\hypvitem{complete2} From \hyp{exsym2} it follows that $\theta([\sym i\notin \dom(\sym h) \land \sym i\notin \sym d])=\vtrue$, which completes our goal \goal{getsym}.
\end{hypvlist}
...
\hypvitem{complete3} From \hyp{exsym3} it follows that $\theta([\sym i\notin \dom(\sym h_c\cup\sym h_s)])=\vtrue$, which completes our goal \goal{getsym}.
\end{hypvlist}
