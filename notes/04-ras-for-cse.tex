...
Note that here we work with the \emph{option} version $\St^?$ of the states $\St$, which extends the carrier set with a $\bot_{\St^?}$ element. If the core of a state $|\st|$ is $\bot$, the core of its option is $|\st^?|=\bot_{\St^?}$, with $\bot_{\St^?}$ \emph{inside} the carrier set of the state model (whereas $\bot\notin \St$). Finally, the composition of any state with $\bot_{\St^?}$ is that state.
...
\quad \ppexec{\hat\alpha}{\sst,\sins}{o,\sst',\souts,\pc} \land \SAT(\pc) \land \theta',s,\st'\models \sst' \land\\
...
\expeval[\hat s,\pc]{\souts}\rightsquigarrow(\outs,\pc')\land \expeval[\hat s,\pc']{\sins}\rightsquigarrow(\ins,\pc'')\implies\\
...
\begin{array}{l}
\ppexec{\alpha}{\sst,\sins}{o, \sst', \souts, \pc'} \land o \neq \Miss \land \sst'\disj\sst_f\implies\\
\quad\sst \disj \sst_f \land \ppexec{\alpha}{\sst\cdot\sst_f, \sins}{o, \sst'\cdot\sst_f, \souts, \pc'}
\end{array}
\end{equation}
...
\qquad (\theta(\pc_f)=\vtrue \land \expeval{\sins} = \ins \land \expeval{\souts}=\outs) \implies\\
...
\quad \theta(\pc\land\pc_f)=\vtrue \Rarr \expeval{\sins}=\ins \Rarr \expeval{\souts}=\outs \Rarr \\
...
\begin{array}{l}
\expeval{\sins}=\ins \land \expeval{\souts}=\outs~\land \theta,s,\st\models\sst \land \st_\delta \models_\Delta \corepred{\delta}{\ins}{\outs} \land \st \disj \st_\delta \implies\\
...
\and\inferrule[DynPMapProd]{\ppget{s,i}{i',s_{i'},\pc} \\ \ppprod{s_{i'},\delta,\sins,\souts}{s'_{i'},\pc} \\ \ppset{s,i',s'_{i'}}{s'}}{\ppprod{s,\delta,i::\sins,\souts}{s', (i = i')::\pc}}
...
$\statemodel{Injector}(\mmdl, I)$ adds hooks into \consume, \produce{} and \execac{}, allowing the developer to apply transformations to the inputs or outputs of these functions. This is also convenient when implementing a state model that needs to satisfy a pre-existing interface. For instance, this makes re-ordering the arguments of certains actions trivial, or allows treating an out-value as an in-value that makes the branch vanish if it doesn't correspond (this is the case for the \code{domainset} predicate in JSIL).
