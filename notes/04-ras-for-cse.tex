...
\and\inferrule[PProductLProd]{(x,y)=\mathit{unwrap}(s) \\ \ppprod{x,\delta_l,\sins,\souts}{x',\pc} \\ s'=\mathit{wrap}(x',y)}{\ppprod{s,\delta_l,\sins,\souts}{s',\pc}}
...
The partial map transformer, $\PMap(I,\mmdl)$, allows modelling partial finite maps. It receives a domain $I$, and the codomain memory model $\mmdl$. 
\begin{align*}
...
\begin{align*}
	\text{Given }&\mathit{update}(h, i, s) = \begin{cases}
		h[i \leftarrow s] &\If~s\neq \bot\\
		h[i \not\leftarrow] &\Otherwise
	\end{cases}\\
	&\mathit{dom\_add}(d, i) = \begin{cases}
		\bot &\If~d=\bot\\
		d\uplus \{i\} &\Otherwise
	\end{cases}\\
	&i\in d^? \defeq \begin{cases}
		\vtrue &\If~d^?=\bot\\
		i\in d&\Otherwise
	\end{cases}
\end{align*}
\begin{mathparpagebreakable}
\inferrule[PMapAction]{i'\in\dom(h) \\ s=h(i') \\ \ppexec{\alpha}{s,\sins}{o,s',\souts,\pc} \\ h' = \mathit{update}(h, i', s')}{\ppexec{\alpha}{(h, d),i::\sins}{o,(h', d),i'::\souts, (i = i')::\pc}}
\and\inferrule[PMapActionBot]{i\not\in\dom(h)\\d=\bot\lor d\neq\dom(h) \\ \ppexec{\alpha}{\bot,\sins}{o,s',\souts,\pc} \\ h'=\mathit{update}(h,i,s')}{\ppexec{\alpha}{(h, d),i::\sins}{o,(h',d),i::\souts, (i\notin\dom(h)\land i\in d)::\pc}}
\and\inferrule[PMapActionOutOfBounds]{d\neq\bot \\ d=\dom(h)}{\ppexec{\alpha}{(h, d),\sins}{\Err, (h,d), [], [i\notin d]}}
\and\inferrule[PMapAlloc]{i \notin \SV \\ i \notin \dom(h) \\ i \notin d \\ s=\code{instantiate}(\sins) \\ h' = h[i\leftarrow s] \\ d' = \mathit{dom\_add}(d, i)}{\ppexec{\alloc}{\SV, (h,d), \sins}{\Ok, (h', d'), [i], []}}
\and\inferrule[PMapCons]{i'\in\dom(h) \\ s=h(i') \\ \ppcons{s,\delta,\sins}{o,s',\souts,\pc} \\ h' = \mathit{update}(h, i', s')}{\ppcons{(h, d),\delta,i::\sins}{o,(h', d),\souts, (i = i')::\pc}}
\and\inferrule[PMapConsBot]{i\not\in\dom(h)\\d=\bot\lor d\neq\dom(h) \\ \ppcons{\bot,\delta,\sins}{o,s',\souts,\pc} \\ h'=\mathit{update}(h,i,s')}{\ppcons{(h, d),\delta,i::\sins}{o,(h',d),\souts, (i\notin\dom(h))::\pc}}
\and\inferrule[PMapConsIncompat]{d\neq\bot \\ d=\dom(h)}{\ppcons{(h, d),\delta,\sins}{\LFail, (h,d), [], [i\notin d]}}
\and\inferrule[PMapConsDomainSet]{d\neq \bot}{\ppcons{(h,d),\domainset,[]}{\Ok,(h,\bot), [d],[]}}
\and\inferrule[PMapConsDomainSetMiss]{}{\ppcons{(h,\bot),\domainset,[]}{\Miss,(h,\bot), [\code{`domainset'}], []}}
\and\inferrule[PMapProd]{i'\in\dom(h) \\ s=h(i') \\ \ppprod{s,\delta,\sins,\souts}{s',\pc} \\ h' = \mathit{update}(h, i', s')}{\ppprod{(h, d),\delta,i::\sins,\souts}{(h', d), (i = i')::\pc}}
\and\inferrule[PMapProdBot]{i\not\in\dom(h)\\d=\bot\lor d\neq\dom(h) \\ \ppprod{\bot,\delta,\sins,\souts}{s',\pc} \\ h'=\mathit{update}(h,i,s')}{\ppprod{(h, d),\delta,i::\sins,\souts}{(h',d),(i\notin\dom(h))::\pc}}
\and\inferrule[PMapProdDomainSet]{}{\ppprod{(h,\bot),\domainset,[],[d]}{(h,d),[\dom(h)\subseteq d]}}
...
\note{I'm actually not sure allocation with a $\bot$ domainset is sound!! Because you lose information you can't get back so probably not EX and probably breaks some axiom. \textbf{Figure it out, prove it, outline it.}}
...
An interesting thing to outline here is that any action execution and predicate consumption and production can branch on the index. For instance, if the state is $(\{ 1\mapsto y \}, \{ 1,2\})$ and a \load{} action is executed with an unconstrained symbolic index $\hat x$, then three branches are created: one where $\hat x = 1$ and the action is executed on the cell $y$, one where $\hat x = 2$ and the action is executed on $\bot$, and finally one where $\hat x\notin \{1,2\}$, and an \Err{} is raised for out of bounds access. This outlines a key difficulty of \PMap{}, that will be discussed more in depth later: implementation-wise, it is a complex and expensive transformer that has the potential to create many branches -- this makes it an important target for optimisation.
...
If we are using PCMs, $\Ex({1})$ is defined as $\ex{1}~|~0$, with $0$ the unit of the PCM. Note here that executing actions, consuming and producing doesn't return an element of $\St^?$, but of $\St$ directly, since the $0$ is already in its carrier set; one has no reason to extend it with $\bot$. The engine first consumes the \exP{} predicate, at address $1$ -- this means the pointed-to state becomes $0$ (or empty), and the bigger (or outer) state becomes $(\{ 1\mapsto 0\}, \{ 1\})$. The \domainset{} predicate is then succesfuly consumed too, leaving $(\{ 1\mapsto 0\},\bot)$. Now, the post-condition is produced onto the state, as calling the function modified the state of our program. First, the engine thus produces \corepred{\exP}{2}{1}, which adds a binding to \PMap{} and creates the cell: our bigger state is now $(\{ 1\mapsto 0, 2\mapsto \ex1\},\bot)$. Finally, \corepred{\domainset}{}{\{2\}} is produced onto the state, however this \emph{doesn't succeed}. Indeed, the bindings of the heap are $\{1,2\}$, which are not a subset of the produced domainset $\{2\}$. This is because the engine cannot tell appart empty ($0$) states from non-empty states. Of course one could handle this by checking for $0$, or by explicitly excluding units from the codomain of \PMap{}. This however makes rules and definitions more complex and prone to error, as it is easy to forget to check for units.

If this examples is now reproduced with RAs, the consumption of \corepred{\exP}{1}{1} makes the cell return $\bot$, which cannot be added into the heap -- the \PMap{} must thus remove the binding, and the postcondition can be produced properly, as the bindings of the heap is only $\{2\}$. RAs are a solution to the above problem, as they facilitate the sound construction of state models and state model transformers, by allowing unitless state models.


%ensures there is only one observably empty state: $\bot$. Indeed, with PCMs a partial map that points to $0$ is possible, as $0$ is part of the carrier set of $\mmdl.\Sigma$ -- this is not the case in RAs, where $\bot$ is defined outside of the carrier set, making the representation $i\mapsto\bot$ invalid.
