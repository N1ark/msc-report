...
We present here the rules for this technique. We \highlight{highlight} the new rule and condition, noting the last two rules are unchanged.

\begin{mathparpagebreakable}
\color{blue}
\inferrule[SyntacticPMapGetMatch]{(h,d)=\mathit{unwrap}(s) \\ i\in\dom(h) \\ s_{i}=h(i)}{\ppget{s, i}{i, s_{i}, []}}
\color{black}
\and\inferrule[SyntacticPMapGetBranch]{(h,d)=\mathit{unwrap}(s) \\ \highlight{i\notin\dom(h)} \\ i'\in\dom(h) \\ s_{i'}=h(i')}{\ppget{s, i}{i', s_{i'}, [i=i']}}
\color{gray}
...
\end{align*}
\begin{mathparpagebreakable}
\inferrule[SplitPMapGetMatchCon]{(h_c,h_s,d)=\mathit{unwrap}(s) \\ \highlight{\code{is\_concrete}~i} \\ i\in\dom(h_c) \\ s_{i}=h_c(i)}{\ppget{s, i}{i, s_{i}, []}}
...
We now present the rules for this state model; in particular, we again only need to concern ourselves with the \code{get} and \code{set} internal methods; actions and predicate consumption and production remain the same.
\begin{mathparpagebreakable}
\inferrule[ALocPMapGetMatch]{(h,d)=\mathit{unwrap}(s) \\\highlight{a = \code{to\_aloc}~i} \\ \highlight{a \neq \bot} \\ a\in\dom(h) \\ s_{a}=h(a)}{\ppget{s, i}{\aloc{a}, s_{a}, []}}
\and\inferrule[ALocPMapGetMatchBot]{(h,d)=\mathit{unwrap}(s) \\\highlight{a = \code{to\_aloc}~i} \\ \highlight{a \neq \bot} \\ a\notin\dom(h) \\ d\neq\bot}{\ppget{s, i}{\aloc{a}, \bot, [\aloc{a}\in d]}}
\and\inferrule[ALocPMapGetNewLoc]{(h,d)=\mathit{unwrap}(s) \\ \highlight{a = \code{to\_aloc}~i} \\ \highlight{a = \bot} \\  d=\bot \\ \highlight{a'=\code{fresh\_aloc ()}}}{\ppget{s, i}{\aloc{a'}, \bot, [i=\aloc{a'}]}}
\and\inferrule[ALocPMapSetSome]{(h,d)=\mathit{unwrap}(s) \\ \highlight{a = \code{to\_aloc}~i} \\ \highlight{a\neq\bot} \\ s_i\neq\bot \\ h'=h[a\leftarrow s_i] \\ s'=\mathit{wrap}(h', d)}{\ppset{s,i,s_i}{s'}}
\and\inferrule[ALocPMapSetNone]{(h,d)=\mathit{unwrap}(s) \\ \highlight{a = \code{to\_aloc}~i} \\ \highlight{a\neq\bot} \\ s_i=\bot \\ h'=h[a\not\leftarrow] \\ s'=\mathit{wrap}(h',d)}{\ppset{s,i,s_i}{s'}}
\end{mathparpagebreakable}
