...
What is gained from using transformers is a shorter implementation, as shown in \autoref{fig:wisl-locs}: only the amount of lines of code needed to instantiate the WISL state model is minuscule. There are more LOCs needed as a whole, but the developer of the state model doesn't need to worry about the size of the library of course.
