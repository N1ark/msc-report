...
Gillian having the unique property of being parametric on the state model makes it particularly well suited for the idea of state model constructions we will define later in this project, as indeed any construction can be handed to it and used seamlessly. If the state model was hard-coded, then one would need to modify the engine's core for each construction.
