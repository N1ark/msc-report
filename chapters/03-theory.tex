...
Furthermore, Iris is a battle-tested logic\footnote{See \url{https://iris-project.org/\#publications}, where more than a hundred publications are cited as using Iris, allowing for the verification of, among others, Scala, OCaml, Rust, WASM, Go or C.}, that comes with plenty proofs and properties making them easy to use and adapt, whereas PCMs can prove unwieldy even for simpler state models as shown previously.
...
It is worth noting that \emph{partial} RAs are equivalent to regular RAs, \emph{so long as $\irisval$ always holds for valid states}\footnote{This, to our knowledge, is the case for all of the simpler RAs defined in Iris: $\Ex$, $\Ag_0$, $\Sum$, $\Product$, etc.}. Indeed, compositions that yield $\lightning$ can be made undefined, and the validity function removed, to gain partiality, and inversely to go back to the Iris definition.
...
We may compare the definition of $\Ex$, presented earlier, for partial RAs and Iris RAs -- the main difference is the removal of $\irisval$ and the $\lightning$ element, making the definition simpler.

\noindent\begin{minipage}{.5\linewidth}
\begin{align*}
	\Ex_{\textsc{Iris}}(X) &\defeq \ex{x:X} ~|~ \lightning \\
	\irisval(a) &\defeq a \neq \lightning \\
	|\ex{a}| &\defeq \bot\\
	a \cdot b &\defeq \lightning
...
\begin{align*}
	\Ex_{\textsc{Partial}}(X) &\defeq \ex{x:X} \\
	|\ex{a}| &\defeq \bot\\
...
The above two axioms are what enable the traditional frame rule of separation logic, but bringing it to operational semantics\note{(Does this make any sense?)}. For frame subtraction, if executing an action on a state from which a frame was removed yields a non-\Miss, then that action did not that frame, and the frame can be added to the result. For frame addiction, which is needed in UX, we go backwards again: if a frame can be added to the state after executing an action and the action doesn't cause a \Miss, then it can also be added to the state before without interfering with the action execution.
...
The only difference for \Ex{} between RAs and PCMs is the absence of a $0$, which is instead $\bot$; in fact $\Ex_{\textsc{RA}}(X)^?$ is strictly equivalent to $\Ex_{\textsc{PCM}}(X)$.
