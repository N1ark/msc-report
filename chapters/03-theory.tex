...
Now consider the \code{dispose} function, as seen in \autoref{fig:example-dispose-freeable}.\footnote{For these examples we only use concrete values, to make them simpler and omit substitutions that happen with symbolic values -- the same results would also hold when using symbolic values, of course.} Given an owned memory value storing $1$, it simply frees it. This function is called by \code{call\_dispose}, which does just that. It follows that these two functions should be succesfully verified, as it is fairly obvious that they fullfil their specifications.
